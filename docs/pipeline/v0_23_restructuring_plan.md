...
| 任务 | 状态 | 说明 |
|------|------|------|
| c.1: storage/sqlite.py → data/models.py | ✅ 完成 | 268 行模型代码提取 |
| c.2: storage/sqlite.py → data/repositories.py | ⏭️ 跳过 | 490 行可接受 |
| c.3: llm/evaluator.py → agent/prompt_builder.py | ⏭️ 跳过 | 450 行可接受 |
| c.4: llm/evaluator.py → agent/json_parser.py | ⏭️ 跳过 | 方法耦合紧密 |
| c.5: pipeline.py → pipeline/source_factory.py | ⏭️ 跳过 | 358 行可接受 |
| c.6: contracts.py → core/enums.py | ⏭️ 跳过 | 313 行可接受 |

**验证**: pytest 52 passed, 所有 import 路径可用, 结果等价验证通过
