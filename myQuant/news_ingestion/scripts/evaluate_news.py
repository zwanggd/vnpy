...
Usage::
    ...
    conda run -n vnpy43 python myQuant/news_ingestion/scripts/evaluate_news.py \\
...
from myQuant.news_ingestion.profiles.stock_profiles import (  # noqa: E402
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # ── Persist stock profiles ───────────────────────────────────────────
    from myQuant.news_ingestion.profiles.stock_profiles import DEFAULT_STOCK_PROFILES, persist_discovered_stock_profiles

    market_db_path = str(Path("~/.vntrader/database.db").expanduser())
    if Path(market_db_path).exists():
        try:
            persist_discovered_stock_profiles(repo, market_db_path, profiles=DEFAULT_STOCK_PROFILES)
            print(f"[profile] Stock profiles persisted to {db_path}", file=sys.stderr)
        except Exception as exc:
            print(f"[profile] Stock profile persist skipped: {exc}", file=sys.stderr)
    ...
    def _get_profile(vt_symbol: str) -> StockProfile | None:
        return DEFAULT_STOCK_PROFILES.get(vt_symbol)

    if args.workers == 1:
        # Sequential
        for mapped, news_item in tqdm(items, desc="LLM evaluating", unit="item"):
            try:
                profile = _get_profile(mapped.vt_symbol)
                llm_run, llm_output, signal = evaluator.evaluate(mapped, news_item, profile=profile)
        ...
        def _evaluate_one(mapped, news_item, profile):
            ev = _get_evaluator()
            return ev.evaluate(mapped, news_item, profile=profile), ev.attempt_records

        with concurrent.futures.ThreadPoolExecutor(
            max_workers=args.workers,
        ) as executor:
            futures = {
                executor.submit(_evaluate_one, m, ni, _get_profile(m.vt_symbol)): (m, ni)
                for m, ni in items
            }
            for future in tqdm(
