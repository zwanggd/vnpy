#!/usr/bin/env python3
"""A0 baseline on 50 items for label quality comparison."""
...
def do(i, db_id, title):
    ...
    try:
        ...
        valid = bool(parsed and all(
            k in parsed for k in ["event", "impact_direction", "impact_strength", "confidence"]
        ))
        results[i] = {
        ...
        results[i] = {
            "idx": i, "total_latency_ms": (time.perf_counter() - t0) * 1000,
            "json_parse_success": False, "error": str(e)[:200],
        }
        done[0] += 1
        print(f"  [A0_50] [{done[0]:2d}/50] FAIL: {type(e).__name__}", flush=True)
...
print(f"\nA0 50-item baseline (4 workers):")
print(f"  Success: {len(valid)}/{len(results)} ({100*len(valid)/len(results):.0f}%)")
print(f"  Latency: avg={sum(elapsed)/len(elapsed):.0f}ms p50={sorted(elapsed)[len(elapsed)//2]:.0f}ms p90={sorted(elapsed)[int(len(elapsed)*0.9)]:.0f}ms")
